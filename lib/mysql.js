  ...
  async.series([
    function(next) {
      ...
      self.query(sql.query, sql.values, function(err, rows) {
        ...
        if (rows && rows.length) results.total = rows[0]['_count(*)'] || 0;
        next();
      });
    },
    function(next) {
      ...
      self.query(sql.query, sql.values, function(err, rows) {
        ...
        if (rows && rows.length) {
          ...
          if (sql.additionalModels) {
            for (var plural in sql.additionalModels) {
              var relatedModel = sql.additionalModels[plural];
...
Model.save = function(fn) {
  ...
  if (this.attrs[this.model.primaryKey]) {
...
Model.buildSQL = function(query) {
  ...
  if (extras.included) {
    sql.additionalModels = extras.included;
  }
...
function prepareQuery(Model, query) {
  ...
  if (query.include) {
    ...
    include.forEach(function(relation) {
      ...
      extras.included[relation.as] = relation.model;
    });
  }
  // Values
  if (query.values) {
...
function stripTableName(attrs, table) {
  for (var attr in attrs) {
    if (attr.indexOf(table + '_') === 0) {
      attrs[attr.replace(table + '_', '')] = attrs[attr];
      delete attrs[attr];
    }
  }
  return attrs;
