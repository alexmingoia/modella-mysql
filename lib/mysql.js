...
function prepareQuery(Model, query) {
  ...
  if (query.where) {
    for (var key in query.where) {
      if (Model.relations[query.where[key]]) {
        fkWhere = query.where[key];
        relation = Model.relations[query.where[key]];
      }
    }
  }
  if (relation) {
    if (relation.through) {
      query.innerJoin = {};
      query.innerJoin[relation.throughTable] = {};
      query.innerJoin[relation.throughTable][relation.throughKey] = '$' + Model.tableName + '.' + Model.primaryKey + '$';
      query.where[relation.throughTable + '.' + relation.foreignKey] = fkWhere;
    }
    else {
      query.where[relation.foreignKey] = fkWhere;
    }
  }
  // Values
  if (query.values) {
