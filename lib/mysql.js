 ...
 *
 * Available settings not listed here are specified in the felixge/node-mysql
 * documentation.
 *
...
function plugin(settings) {
  ...
  return function(Model) {
    ...
    if (settings.tableName) {
      Model.tableName = settings.tableName;
    }
    if (!Model.tableName) {
    ...
    Model.on('initializing', function(model, attrs) {
      ...
      for (var key in attrs) {
        ...
        if (model.model.attrs[key].type == 'date') {
          if (typeof attrs[key] != 'object') {
            if (isNaN(attrs[key])) {
              attrs[key] = new Date(model.attrs[key]);
            }
            else {
              attrs[key] = new Date(model.attrs[key] * 1000);
            }
          }
        }
      }
...
Model.all = function(query, callback) {
  ...
  if (query.limit > this.db.settings.maxLimit) {
    query.limit = this.db.settings.maxLimit;
  }
  var self = this;
  var results = {
    data: [],
...
Model.save = function(fn) {
  var model = this;
  this.model.emit('mysql before save', this, this.attrs);
  this.emit('mysql before save', this.attrs);
  ...
  this.model.query(sql.query, sql.values, function(err, rows, fields) {
    ...
    this.emit('mysql after save', model, body);
    model.emit('mysql after save', body);
    fn(null, body);
  });
};
...
Model.update = function(fn) {
  var model = this;
  var attrs = this.changed();
  this.model.emit('mysql before update', this, attrs);
  this.emit('mysql before update', attrs);
  var where = {};
  ...
  this.model.query(sql.query, sql.values, function(err, rows, fields) {
    if (err) return fn(err);
    this.emit('mysql after update', model, fields);
    model.emit('mysql after update', fields);
    fn(null, fields);
  });
};
...
proto.toJSON = function() {
  ...
  for (var attr in json) {
    if (attrs[attr].type == 'date' && this[attr] instanceof Date) {
      json[attr] = this[attr]().toISOString();
    }
  }
  return json;
};
...
function formatAttrs(model, attrs) {
  for (var attr in attrs) {
    var val = attrs[attr];
    if (model.model.attrs[attr] && model.model.attrs[attr].type == 'date') {
  ...
  if (query.values) {
    ...
    for (var key in values) {
      if (Model.attrs[key] && Model.attrs[key].dataFormatter) {
        ...
        switch (Model.attrs[key].columnType) {
          case 'datetime':
